  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (selectedFilters.length > 0) {
        ...
        selectedFilters.forEach(filter => params.append('editors', filter));
        url += `?${params.toString()}`;
      }
      ...
      if (result.available_editors) {
        setAvailableOptions(result.available_editors);
      }
    } catch (error) {
    ...
    <div className={styles.container}>
      ...
      <div className={styles.filterContainer}>
        <div className={styles.filterTitle}>
          Filter Editors ({availableOptions.length} available)
        </div>
      ...
      <div className={styles.dataContainer}>
        {loading && <div className={styles.loading}>Loading editors data...</div>}
        
        {data.error && <div className={styles.error}>Error: {data.error}</div>}

        {data.editors_daily && (
          <div className={styles.chartSection}>
            <h3>Editors Usage (Daily)</h3>
            <div className={styles.chartContainer}>
              {(() => {
                const chartData = createLineChart(
                  data.editors_daily.data,
                  'date',
                  ['total_engaged_users', 'total_code_acceptances', 'total_code_suggestions', 'total_chat_acceptances', 'total_chat_turns']
                );
        ...
        {data.top_editors && (
          <div className={styles.chartSection}>
            <h3>Top Editors by Engaged Users</h3>
            <div className={styles.chartContainer}>
              {(() => {
                const chartData = createPieChart(data.top_editors.data);
        ...
        {data.editors_weekly && (
          <div className={styles.chartSection}>
            <h3>Editors Usage (Last 7 Days)</h3>
            <div className={styles.chartContainer}>
              {(() => {
                const chartData = createLineChart(
